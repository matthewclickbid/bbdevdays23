...
async function fetchClickBid() {
  let response = await fetch(CB_ROOT_API+'/bids/'+cbTs,{method:'GET',
    headers: {
...
async function fetchREData() {
  let response = await fetch(RE_ROOT_API+'/gift/v1/gifts?date_added='+reTs,
  {
    method:'GET',
    headers: {
...
async function getREToken() {
  ...
  let response = await fetch(RE_ROOT_REDIRECT,
  {
    ...
    headers: {
      ...
      'Authorization': 'Basic ' + window.btoa(`${RE_CLIENT_ID}:${RE_CLIENT_SECRET}`),
    },
...
function randomHeart(amount) {
  ...
  span.innerHTML = `
      <span class="heartAmount">$${formattedNumber}</span>
  ...
  tl.to(span, {
    ...
    duration: gsap.utils.random(6, 8, .2),
    ease: Power0.easeNone
  }, -.3)
